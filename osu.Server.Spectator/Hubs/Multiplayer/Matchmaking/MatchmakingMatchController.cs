...
namespace osu.Server.Spectator.Hubs.Multiplayer.Matchmaking
{
    ...
    public class MatchmakingMatchController : IMatchController
    {
        ...
        public MultiplayerPlaylistItem CurrentItem => room.Playlist.Single(i => i.ID == room.Settings.PlaylistItemId);
        ...
        {
            await eventNotifier.OnMatchRoomStateChangedAsync(room.RoomID, room.MatchState);
        ...
        public async Task HandleUserRequest(MultiplayerRoomUser user, MatchUserRequest request)
        {
            switch (request)
            {
                case MatchmakingAvatarActionRequest avatarAction:
                    await eventNotifier.OnNewMatchEventAsync(room.RoomID, new MatchmakingAvatarActionEvent
                    {
                        UserId = user.UserID,
        ...
        public async Task HandleUserJoined(MultiplayerRoomUser user)
        {
            switch (state.Stage)
            {
                case MatchmakingStage.WaitingForClientsJoin:
                    await eventNotifier.OnPlayerJoinedMatchmakingAsync(room.RoomID, user.UserID);
